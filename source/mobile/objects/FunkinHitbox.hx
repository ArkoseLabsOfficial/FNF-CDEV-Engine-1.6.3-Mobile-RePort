...
class FunkinHitbox extends Hitbox {
	...
	public function new(?mode:String, ?showHints:Bool):Void
	{
		...
		var Custom:String = mode != null ? mode : CDevConfig.saveData.hitboxMode;
		...
		if (CDevConfig.saveData.mobileExtraKeys == 1 && MobileConfig.hitboxModes.get(Custom).single != null)
			currentHint = MobileConfig.hitboxModes.get(Custom).single;
		if (CDevConfig.saveData.mobileExtraKeys == 2 && MobileConfig.hitboxModes.get(Custom).double != null)
			currentHint = MobileConfig.hitboxModes.get(Custom).double;
		if (CDevConfig.saveData.mobileExtraKeys == 3 && MobileConfig.hitboxModes.get(Custom).triple != null)
			currentHint = MobileConfig.hitboxModes.get(Custom).triple;
		if (CDevConfig.saveData.mobileExtraKeys == 4 && MobileConfig.hitboxModes.get(Custom).quad != null)
			currentHint = MobileConfig.hitboxModes.get(Custom).quad;
		if (CDevConfig.saveData.mobileExtraKeys != 0 && MobileConfig.hitboxModes.get(Custom).hints != null)
		...
		for (buttonData in currentHint)
		{
			...
			if (CDevConfig.saveData.mobileExtraKeys == 0 && buttonData.extraKeyMode == 0 ||
			   CDevConfig.saveData.mobileExtraKeys == 1 && buttonData.extraKeyMode == 1 ||
			   CDevConfig.saveData.mobileExtraKeys == 2 && buttonData.extraKeyMode == 2 ||
			   CDevConfig.saveData.mobileExtraKeys == 3 && buttonData.extraKeyMode == 3 ||
			   CDevConfig.saveData.mobileExtraKeys == 4 && buttonData.extraKeyMode == 4 ||
			...
			for (i in 1...5) {
				...
				if (buttonData.button == buttonString && buttonReturn == null)
					buttonReturn = CDevConfig.saveData.mobileExtraKeyReturns[i-1];
			}
			if (addButton)
	...
	override public function createHint(name:Array<String>, uniqueID:Int, x:Float, y:Float, width:Int, height:Int, color:Int = 0xFFFFFF, ?returned:String):MobileButton
	{
		...
		hint.antialiasing = CDevConfig.saveData.antialiasing;
